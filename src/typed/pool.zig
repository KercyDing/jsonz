...
/// An allocator that hands out `buffer` before falling back to another
/// allocator.
///
/// `buffer` must stay valid as long as its allocations are in use; fallback
/// allocations follow `fallback_allocator`'s ownership rules.
